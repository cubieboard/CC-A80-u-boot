...
int disp_sys_clk_set_rate(const char *id, unsigned long rate)
{
	...
	if(!strcmp(id, "pll_video0")) {
		...
		if(297000000 == rate)
			smc_writel(0x80016302, 0x01c20010);
		else {
			if(rate >= 600000000) {
				// 24 * n / (div+1) < 1.2 G
		...
		if(297000000 == rate)
			smc_writel(0x80016302, 0x01c2004c);
		else {
			if(rate >= 600000000) {
				// 24 * n / (div+1) < 1.2 G
				factor_n = rate / 12000000;
		...
		smc_writel(reg_val, 0x01c20048);
	} else if(!strcmp(id, "lcd1")) {
		unsigned int div = 297000000 / rate;
		unsigned int reg_val = smc_readl(0x01c2011c);

		reg_val &= ~0xf;
		reg_val |= (div-1);

		smc_writel(reg_val, 0x01c2011c);
	} else if(!strcmp(id, "hdmi")) {
		unsigned int div = 297000000 / rate;
		unsigned int reg_val = smc_readl(0x01c20150);

		reg_val &= ~0xf;
		reg_val |= (div-1);

		smc_writel(reg_val, 0x01c20150);
	}
...
unsigned long disp_sys_clk_get_rate(const char *id)
{
	...
	if(!strcmp(id, "pll_video0")) {
		...
		unsigned int p;
		unsigned int reg_val = smc_readl(0x01c20010);
		...
		unsigned int factor_n;
		unsigned int p;
		unsigned int reg_val = smc_readl(0x01c2004c);
		...
		/* fix div2(1), div1(0): rate = 24 * n / (div2+1) / (div1+1) */
		unsigned int factor_n;
		unsigned int reg_val = smc_readl(0x01c20048);
...
int disp_sys_clk_enable(const char *id)
{
	...
	if(!strcmp(id, "pll_video0")) {
		...
		/* reset */
		reg_val = smc_readl(0x01c202c4);
		reg_val |= 0x10;
		smc_writel(reg_val, 0x01c202c4);
		/* bus gating */
		reg_val = smc_readl(0x01c20064);
		reg_val |= 0x10;
		smc_writel(reg_val, 0x01c20064);
		/* module gating */
		reg_val = smc_readl(0x01c20118);
		reg_val |= 0x80000000;
		smc_writel(reg_val, 0x01c20118);
	} else if(!strcmp(id, "lvds")) {
		smc_writel(1, 0x01c202c8);
		...
		/* reset */
		reg_val = smc_readl(0x01c202c0);
		reg_val |= 0x2;
		smc_writel(reg_val, 0x01c202c0);
		/* bus gating */
		reg_val = smc_readl(0x01c20060);
		reg_val |= 0x2;
		smc_writel(reg_val, 0x01c20060);
		/* module gating */
		reg_val = smc_readl(0x01c20168);
		reg_val |= 0x3;//div 4(force)
		smc_writel(reg_val, 0x01c20168);
		reg_val |= 0x8000000;//src video pll
		smc_writel(reg_val, 0x01c20168);
		reg_val |= 0x80000000;//en
		smc_writel(reg_val, 0x01c20168);
		/* module gating(phy) */
		reg_val = smc_readl(0x01c2016c);
		reg_val |= 0x80000000;
		smc_writel(reg_val, 0x01c2016c);
	} else if(!strcmp(id, "lcd1")) {
		/* pll_video1 */
		reg_val = smc_readl(0x01c2004c);
		...
		smc_writel(reg_val, 0x01c20064);
		/* module gating */
		reg_val = smc_readl(0x01c20150);
		reg_val |= 0x80000000;
		smc_writel(reg_val, 0x01c20150);
		reg_val = smc_readl(0x01c20154);
		reg_val |= 0x80000000;
		smc_writel(reg_val, 0x01c20154);
	}
