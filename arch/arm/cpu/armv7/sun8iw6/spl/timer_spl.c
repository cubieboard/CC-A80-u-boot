...
static void __delay_null(unsigned long time);
static void __msdelay_nssecure(unsigned long ms);
static void __usdelay_nssecure(unsigned long us);
static void __msdelay_secure(unsigned long ms);
static void __usdelay_secure(unsigned long us);
...
int timer_init(void)
{
	...
	else
	{
		...
		writel(reg_val, TMRC_CTRL(0));
		writel(0xffffffff, TMRC_INTV(0));
		reg_val = 0;
		do
		{
			reg_val = readl(TMRC_CTRL(0));
		}
		while(reg_val & 0x02);
		writel(readl(TMRC_CTRL(0)) | 1, TMRC_CTRL(0));
