...
int sunxi_clock_get_corepll(void)
{
	...
	int 	clock;
    //pll = 24M*N/P
	reg_val  = smc_readl(CCMU_PLL_C0CPUX_CTRL_REG);
...
int sunxi_clock_get_axi(void)
{
	...
	reg_val   = smc_readl(CCMU_CPUX_AXI_CFG_REG);
...
int sunxi_clock_get_ahb(void)
{
	...
	reg_val = smc_readl(CCMU_AHB1_APB1_CFG_REG);
...
static int clk_set_divd(void)
{
	...
	smc_writel(reg_val, CCMU_CPUX_AXI_CFG_REG);
	...
	smc_writel(reg_val, CCMU_AHB1_APB1_CFG_REG);
    #endif
...
static int clk_set_pll_c0_para(int frequency)
{
	...
	smc_writel(reg_val, CCMU_PLL_C0CPUX_CTRL_REG);

      //wait clock stable
#ifndef CONFIG_A73_FPGA
	do
	{
		reg_val = smc_readl(CCMU_PLL_STB_STATUS_REG);
	}
	while(!(reg_val & 0x1));
#endif
...
static int clk_set_pll_c1_para(int frequency)
{
	...
	reg_val  = smc_readl(CCMU_PLL_C1CPUX_CTRL_REG);
	...
	smc_writel(reg_val, CCMU_PLL_C1CPUX_CTRL_REG);
...
int sunxi_clock_get_pll6(void)
{
    ...
    reg_val = smc_readl(CCMU_PLL_PERIPH_CTRL_REG);
    factor_n = ((reg_val >> 8) & 0xff);
